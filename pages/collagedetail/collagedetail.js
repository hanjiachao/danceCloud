...
Page({
  ...
  next_two: function () {
    ...
    moneynum: function (coupon_status,integral,member) {
       var money = ''
       var menbernum = parseFloat(this.data.dance_detail.dg_price) * parseFloat(this.data.member_discount)
       if (member) {
           if (integral) {
           ...
           if (integral) {
               money = (parseFloat(this.data.dance_detail.dg_price) - parseFloat(this.data.proportionprice)).toFixed(2)
               if (this.data.plusmoney != ''  && coupon_status) {
                   money = (parseFloat(money) - this.data.plusmoney).toFixed(2)
               }
               if (money < 0) {
               ...
               money = parseFloat(this.data.dance_detail.dg_price)
               if (this.data.plusmoney != ''  && coupon_status) {
                   money = (parseFloat(money) - this.data.plusmoney).toFixed(2)
               }
               if (money < 0) {
    ...
    voucher: function () {
      if (!this.data.coupon_status) {
        ...
        wx.navigateTo({
            url:'../coupon/coupon?price=' + this.data.dance_detail.dg_price
        })
    },
            ...
            success: function (res) {
                if (res.status != 'SUCCESS') {
                    ...
                    if (res.result.order_status == '已支付') {
                        wx.redirectTo({
                            url:'../ordersuccess/ordersuccess?integral=' + res.result.can_get_integral + '&type=' + '拼团'  + '&order_no=' + res.result.order_no
                        })
                    } else {
                        ...
                        wx.requestPayment(
                            {
                                ...
                                'success':function(res){
                                    wx.redirectTo({
                                        url:'../ordersuccess/ordersuccess?integral=' + can_get_integral + '&type=' + '拼团' + '&order_no=' + order_no
                                    })
                                },
                                'fail':function(res){},
  ...
  onLoad: function (options) {
      ...
      var dg_id = 'info.dg_id'
      this.setData({
          [dg_id]: options.dg_id
      })
      var that = this
      common.ajax({
          url: 'DanceGroup/confirmOrder',
          data: {
              dg_id:options.dg_id
          },
          userinfo:true,
          ...
          success: function (res) {
              ...
              if (res.status != 'SUCCESS') {
                  ...
                  if (res.result.member_status) {
                      var money = (parseFloat(res.result.dance_detail.dg_price) * parseFloat(res.result.member_discount)).toFixed(2)
                      that.setData({
                          member: true,
                          money: money
                      })
                      ...
                      that.setData({
                          money: res.result.dance_detail.dg_price
                      })
                  }
                  that.setData({
